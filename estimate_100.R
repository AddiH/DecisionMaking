...
for (study_name in names(data_list)) {
  ...
  if(n_trials == 100){
    data100trials[[study_name]] <- df
  }
}

combined_df <- do.call(rbind, lapply(names(data100trials), function(study_name) {
  df <- data100trials[[study_name]]
...
save_path <- 'estimations/trial100.rdata'
